...
const getTeamName = (tag) => {
  tag = tag.toLowerCase();
  const teamName = teamTagToTeamMap[tag];
  if (!teamName) return tag;
  return teamName;
}
    ...
    <div className={`player-card ${getRankClass(player.rank)}`}>
      <div className="player-card-inner">
        ...
        <div className="player-info">
          ...
          {player.teamTag ? 
            <p className='player-team'>{getTeamName(player.teamTag)} ({player.teamTag})</p> :
